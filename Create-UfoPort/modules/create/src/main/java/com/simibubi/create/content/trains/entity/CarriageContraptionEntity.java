...
//		SynchedEntityData.defineId(CarriageContraptionEntity.class, AllEntityDataSerializers.CARRIAGE_DATA);
	private static final EntityDataAccessor<Optional<UUID>> TRACK_GRAPH = SynchedEntityData
			.defineId(CarriageContraptionEntity.class, EntityDataSerializers.OPTIONAL_UUID);
	private static final EntityDataAccessor<Boolean> SCHEDULED = SynchedEntityData
			.defineId(CarriageContraptionEntity.class, EntityDataSerializers.BOOLEAN);
...
//		var1.define(CARRIAGE_DATA, new CarriageSyncData());
		var1.define(TRACK_GRAPH, Optional.empty());
		var1.define(SCHEDULED, false);
	}
	...
	// fabric: initial carriageData sync since that's not handled by tracked data
	// anymore
	...
	public boolean isLocalCoordWithin(BlockPos localPos, int min, int max) {
		...
		int coord = axis.choose(localPos.getZ(), localPos.getY(), localPos.getX())
	...
	public static CarriageContraptionEntity create(Level world, CarriageContraption contraption) {
		CarriageContraptionEntity entity = new CarriageContraptionEntity(AllEntityTypes.CARRIAGE_CONTRAPTION.get(),
				world);
	...
	protected void tickContraption() {
		...
		if (!level().isClientSide) {
			...
			boolean shouldCarriageSyncThisTick = carriage.train.shouldCarriageSyncThisTick(level().getGameTime(),
	...
	private void spawnPortalParticles(DimensionalCarriageEntity dce) {
		...
		for (BlockPos pos : particleSlice) {
			...
			CubeParticleData data = new CubeParticleData(.25f, 0, .5f, .65f + (random.nextFloat() - .5f) * .25f, 4,
					false);
	...
	public boolean control(BlockPos controlsLocalPos, Collection<Integer> heldControls, Player player) {
		...
		carriage.train.manualSteer = targetSteer < 0 ? SteerDirection.RIGHT
				: targetSteer > 0 ? SteerDirection.LEFT : SteerDirection.NONE;
	...
	private void displayApproachStationMessage(Player player, GlobalStation station) {
		sendPrompt(player, Lang.translateDirect("contraption.controls.approach_station", Components.keybind("key.jump"),
				station.name), false);
		stationMessage = true;
	}
