...
public class RequestPromiseQueue {
	...
	public CompoundTag write(HolderLookup.Provider registries) {
		...
		Codec.list(RequestPromise.CODEC).encodeStart(ops, flatten(false))
			.resultOrPartial(err -> {})
			.ifPresent(encoded -> tag.put("List", encoded));
		return tag;
	}
	...
	public static RequestPromiseQueue read(CompoundTag tag, HolderLookup.Provider registries, Runnable onChanged) {
		...
		if (tag.contains("List")) {
			RegistryOps<Tag> ops = RegistryOps.create(NbtOps.INSTANCE, registries);
			List<RequestPromise> promises = Codec.list(RequestPromise.CODEC).parse(ops, tag.get("List"))
				.resultOrPartial(err -> {})
				.orElse(List.of());
	...
	public void tick() {
		promisesByItem.forEach((key, list) -> list.forEach(RequestPromise::tick));
	}
