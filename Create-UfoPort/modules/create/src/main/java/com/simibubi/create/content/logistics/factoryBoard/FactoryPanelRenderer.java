...
public class FactoryPanelRenderer extends SmartBlockEntityRenderer<FactoryPanelBlockEntity> {
	...
	public static void renderPath(FactoryPanelBehaviour behaviour, FactoryPanelConnection connection,
		...
		Map<Direction, PartialModel> arrowsMap = AllPartialModels.FACTORY_PANEL_ARROWS;
		Map<Direction, PartialModel> linesMap = AllPartialModels.FACTORY_PANEL_LINES;
		Map<Direction, PartialModel> dottedMap = AllPartialModels.FACTORY_PANEL_DOTTED;
		...
		for (int i = 0; i < path.size(); i++) {
			...
			Direction lookupDir = pathReversed ? direction : direction.getOpposite();
			PartialModel partial;
			if (dots) {
				partial = dottedMap.get(lookupDir);
			} else if (isArrowSegment) {
				partial = arrowsMap.get(lookupDir);
			} else {
				partial = linesMap.get(lookupDir);
			}
