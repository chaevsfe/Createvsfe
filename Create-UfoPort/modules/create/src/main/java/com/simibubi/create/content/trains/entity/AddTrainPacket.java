...
/**
 * S2C packet sent when a new train is registered on the client.
 * Fabric/UfoPort: Equivalent to NeoForge AddTrainPacket; serializes Train data without the add/remove boolean.
 */
public class AddTrainPacket extends SimplePacketBase {

	private Train train;

	public AddTrainPacket(Train train) {
		this.train = train;
	}

	public AddTrainPacket(RegistryFriendlyByteBuf buffer) {
		...
		train.name = Component.Serializer.fromJson(buffer.readUtf(), Create.getRegistryAccess());
	...
	public void write(RegistryFriendlyByteBuf buffer) {
		...
		buffer.writeUtf(Component.Serializer.toJson(train.name, Create.getRegistryAccess()));
		buffer.writeResourceLocation(train.icon.id);
	}

	@Override
	public boolean handle(Context context) {
		context.enqueueWork(() -> CreateClient.RAILWAYS.trains.put(train.id, train));
		return true;
	}
}
