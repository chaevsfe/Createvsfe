...
public class ComputerUtil {
	...
	private static boolean deepEquals(Object fVal, Object iVal) throws LuaException {
		...
		if (fVal instanceof Map<?, ?> fMap && fMap.get("_op") instanceof String op &&
			...
			if (iVal instanceof String inStr && fValue instanceof String fStr) {
				return switch (op) {
					case "glob" -> inStr.matches(globToRegex(fStr));
	...
	private static boolean isArrayLike(Map<?, ?> map) {
		...
		for (Object keyObj : map.keySet()) {
			...
			if (k != (double) k) return false; // not a whole number
	...
	// Fabric: uses ItemStackHandler instead of IItemHandler
	...
	public static Map<String, ?> getItemDetail(ItemStackHandler inventory, int slot) throws LuaException {
		int maxSlots = inventory.getSlotCount();
	...
	/** Converts a simple glob pattern (only * and ?) to a Java regex string. */
