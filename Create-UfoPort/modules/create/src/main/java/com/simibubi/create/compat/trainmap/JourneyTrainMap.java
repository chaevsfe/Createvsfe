...
public class JourneyTrainMap implements IClientPlugin {
	...
	private static boolean isFullscreenOpen() {
		if (jmApi == null) return false;
		UIState state = jmApi.getUIState(UI.Fullscreen);
	...
	public static void tick() {
		if (!AllConfigs.client().showTrainMapOverlay.get() || !isFullscreenOpen()) {
			if (requesting)
