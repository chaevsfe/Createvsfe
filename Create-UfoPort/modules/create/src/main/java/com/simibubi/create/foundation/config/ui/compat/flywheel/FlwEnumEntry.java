...
public class FlwEnumEntry extends FlwValueEntry<Enum<?>> {
	...
	public FlwEnumEntry(String label, Option<Enum<?>> option) {
		...
		cycleLeft = new BoxWidget(0, 0, cycleWidth + 8, 16)
				.withCustomBackground(Theme.c(Theme.Key.PONDER_BACKGROUND_FLAT)).showingElement(l)
		...
		cycleRight = new BoxWidget(0, 0, cycleWidth + 8, 16)
				.withCustomBackground(Theme.c(Theme.Key.PONDER_BACKGROUND_FLAT)).showingElement(r)
				.withCallback(() -> cycleValue(1));
		r.at(cycleWidth - 8, 0);
	...
	public void render(GuiGraphics graphics, int index, int y, int x, int width, int height, int mouseX, int mouseY,
		...
		valueText.at(cycleLeft.getX() + cycleWidth - 8, y + 10, 200)
				.withBounds(width - getLabelWidth(width) - 2 * cycleWidth - resetWidth - 4, 16).render(graphics);
		...
		new BoxElement().withBackground(Theme.c(Theme.Key.PONDER_BACKGROUND_FLAT)).flatBorder(0x01_000000)
				.withBounds(48, 6).at(cycleLeft.getX() + 22, cycleLeft.getY() + 5).render(graphics);
	}
