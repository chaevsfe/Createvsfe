...
public class RedstoneScenes2 {
	...
	public static void roseQuartzLamp(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.world.modifyBlockEntityNBT(comparator, NixieTubeBlockEntity.class,
				nbt -> nbt.putInt("RedstoneStrength", 13));
		scene.idle(25);
		...
		scene.overlay.showControls(
				new InputWindowElement(util.vector.topOf(centerLamp.east(2)), Pointing.DOWN).rightClick().withWrench(),
				20);
		scene.idle(6);
		scene.world.cycleBlockProperty(centerLamp.east(), RoseQuartzLampBlock.POWERING);
		scene.world.toggleRedstonePower(comparator);
		...
		scene.overlay.showControls(
				new InputWindowElement(util.vector.topOf(centerLamp), Pointing.DOWN).rightClick().withWrench(), 20);
		scene.idle(6);
