...
public class ShoppingListItem extends Item {
	public record ShoppingList(@Unmodifiable List<IntAttached<BlockPos>> purchases, UUID shopOwner, UUID shopNetwork) {
		...
		public static final StreamCodec<net.minecraft.network.RegistryFriendlyByteBuf, ShoppingList> STREAM_CODEC = StreamCodec.composite(
			ByteBufCodecs.fromCodecWithRegistries(Codec.list(IntAttached.codec(BlockPos.CODEC))), ShoppingList::purchases,
		...
		public ShoppingList duplicate() {
			return new ShoppingList(new ArrayList<>(purchases.stream()
				.map(ia -> IntAttached.with(ia.getFirst(), ia.getValue()))
