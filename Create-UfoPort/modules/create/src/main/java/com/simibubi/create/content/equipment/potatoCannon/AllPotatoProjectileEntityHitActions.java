...
public class AllPotatoProjectileEntityHitActions {
	...
	public record ChorusTeleport(double teleportDiameter) implements PotatoProjectileEntityHitAction {
		...
		public boolean execute(ItemStack projectile, EntityHitResult ray, Type type) {
			...
			for (int teleportTry = 0; teleportTry < 16; ++teleportTry) {
				...
				double teleportY = Mth.clamp(entityY + (livingEntity.getRandom().nextInt((int) teleportDiameter) - (int) (teleportDiameter / 2)), 0.0D, level.getHeight() - 1);
				double teleportZ = entityZ + (livingEntity.getRandom().nextDouble() - 0.5D) * teleportDiameter;

				// Fabric: no EventHooks.onChorusFruitTeleport — use vanilla randomTeleport directly
	...
	public enum CureZombieVillager implements PotatoProjectileEntityHitAction {
		...
		private static final WorldAttached<net.fabricmc.fabric.api.entity.FakePlayer> ZOMBIE_CONVERTERS =
			new WorldAttached<>(w -> net.fabricmc.fabric.api.entity.FakePlayer.get((ServerLevel) w, ZOMBIE_CONVERTER_NAME));
		...
		public boolean execute(ItemStack projectile, EntityHitResult ray, Type type) {
			...
			net.fabricmc.fabric.api.entity.FakePlayer dummy = ZOMBIE_CONVERTERS.get(world);
