...
public class BezierConnection implements Iterable<BezierConnection.Segment> {
	...
	public BezierConnection(CompoundTag compound, BlockPos localTo) {
		this(Couple.create(NbtFixer.readBlockPosList(compound, "Positions").get(0), NbtFixer.readBlockPosList(compound, "Positions").get(1))
	...
	public CompoundTag write(BlockPos localTo) {
		...
		NbtFixer.writeBlockPosList(compound, "Positions", List.of(tePositions.getFirst(), tePositions.getSecond()));
	...
	/**
	 * Rasterises this bezier curve into a map of (x,z) grid positions to Y heights,
	 * used by the train map overlay renderer.
	 */
	public Map<Pair<Integer, Integer>, Double> rasterise() {
		...
		for (int i = 0; i < segCount; i++) {
			...
			BlockPos pos = BlockPos.containing(railMiddle.add(Vec3.atLowerCornerOf(tePosition)));
			...
			if (alreadyPresent && yLevels.get(key) <= railMiddle.y)
				continue;
			yLevels.put(key, railMiddle.y + tePosition.getY());
			if (alreadyPresent)
				continue;
