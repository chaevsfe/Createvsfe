...
public abstract class SimpleRegistryImpl<K, V> implements SimpleRegistry<K, V> {
	...
	public synchronized V get(StateHolder<K, ?> state) {
		...
		// StateHolder has an 'owner' field accessible via reflection or accessor
		// For Fabric, we use the block directly from BlockState
		K owner;
		if (state instanceof net.minecraft.world.level.block.state.BlockState blockState) {
			owner = (K) blockState.getBlock();
		} else if (state instanceof net.minecraft.world.level.material.FluidState fluidState) {
			owner = (K) fluidState.getType();
		} else {
			throw new UnsupportedOperationException("Unknown StateHolder type: " + state.getClass());
		}
