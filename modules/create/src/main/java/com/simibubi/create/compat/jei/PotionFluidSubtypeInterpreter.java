...
public class PotionFluidSubtypeInterpreter implements IIngredientSubtypeInterpreter<IJeiFluidIngredient> {
	...
	public String apply(IJeiFluidIngredient ingredient, UidContext context) {
		...
		BottleType bottleTypeEnum = ingredient.getFluidVariant().getComponents().get(AllDataComponents.BOTTLE_TYPE) != null
				&& ingredient.getFluidVariant().getComponents().get(AllDataComponents.BOTTLE_TYPE).isPresent()
				? ingredient.getFluidVariant().getComponents().get(AllDataComponents.BOTTLE_TYPE).get()
				: BottleType.REGULAR;
		String bottleType = bottleTypeEnum.toString();
