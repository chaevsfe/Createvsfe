...
public class AddressEditBox extends EditBox {
	...
	public void renderWidget(GuiGraphics pGuiGraphics, int pMouseX, int pMouseY, float pPartialTick) {
		...
		var matrixStack = pGuiGraphics.pose();
		matrixStack.pushPose();
		matrixStack.translate(0, 0, 500);
		destinationSuggestions.render(pGuiGraphics, pMouseX, pMouseY);
		matrixStack.popPose();

		if (destinationSuggestions.suggestions != null)
			return;
		...
		if (pMouseX >= itemX && pMouseX < itemX + 16 && pMouseY >= itemY && pMouseY < itemY + 16) {
			List<Component> promiseTip = List.of(Lang.translate("gui.address_box.clipboard_tip")
