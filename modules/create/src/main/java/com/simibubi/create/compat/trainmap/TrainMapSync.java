...
public class TrainMapSync {
	...
	public enum TrainState {
		...
		public static final StreamCodec<ByteBuf, TrainState> STREAM_CODEC = CreateStreamCodecs.ofEnum(TrainState.class);
	}

	public enum SignalState {
		NOT_WAITING, WAITING_FOR_REDSTONE, BLOCK_SIGNAL, CHAIN_SIGNAL;

		public static final StreamCodec<ByteBuf, SignalState> STREAM_CODEC = CreateStreamCodecs.ofEnum(SignalState.class);
	}

	public static class TrainMapSyncEntry {
		// Manual codec — 10 fields exceeds StreamCodec.composite's 6-field limit
		...
		public static final StreamCodec<FriendlyByteBuf, TrainMapSyncEntry> STREAM_CODEC = new StreamCodec<>() {
			@Override
			public TrainMapSyncEntry decode(FriendlyByteBuf buf) {
				TrainMapSyncEntry e = new TrainMapSyncEntry();
				int posLen = buf.readVarInt();
				e.positions = new Float[posLen];
				for (int i = 0; i < posLen; i++)
					e.positions[i] = buf.readFloat();
				int dimLen = buf.readVarInt();
				e.dimensions = new ArrayList<>(dimLen);
				for (int i = 0; i < dimLen; i++) {
					boolean present = buf.readBoolean();
					e.dimensions.add(present ? ResourceKey.streamCodec(Registries.DIMENSION).decode(buf) : null);
				}
				e.state = TrainState.STREAM_CODEC.decode(buf);
				e.signalState = SignalState.STREAM_CODEC.decode(buf);
				e.fueled = buf.readBoolean();
				e.backwards = buf.readBoolean();
				e.targetStationDistance = buf.readVarInt();
				e.ownerName = buf.readUtf();
				e.targetStationName = buf.readUtf();
				boolean hasWaiting = buf.readBoolean();
				e.waitingForTrain = hasWaiting ? UUIDUtil.STREAM_CODEC.decode(buf) : null;
				return e;
			}

			@Override
			public void encode(FriendlyByteBuf buf, TrainMapSyncEntry e) {
				buf.writeVarInt(e.positions == null ? 0 : e.positions.length);
				if (e.positions != null)
					for (Float f : e.positions)
						buf.writeFloat(f == null ? 0f : f);
				buf.writeVarInt(e.dimensions == null ? 0 : e.dimensions.size());
				if (e.dimensions != null)
					for (ResourceKey<Level> dim : e.dimensions) {
						buf.writeBoolean(dim != null);
						if (dim != null)
							ResourceKey.streamCodec(Registries.DIMENSION).encode(buf, dim);
					}
				TrainState.STREAM_CODEC.encode(buf, e.state);
				SignalState.STREAM_CODEC.encode(buf, e.signalState);
				buf.writeBoolean(e.fueled);
				buf.writeBoolean(e.backwards);
				buf.writeVarInt(e.targetStationDistance);
				buf.writeUtf(e.ownerName);
				buf.writeUtf(e.targetStationName);
				buf.writeBoolean(e.waitingForTrain != null);
				if (e.waitingForTrain != null)
					UUIDUtil.STREAM_CODEC.encode(buf, e.waitingForTrain);
			}
		};
	...
	public static void register() {
		ServerTickEvents.END_SERVER_TICK.register(TrainMapSync::serverTick);
	}
