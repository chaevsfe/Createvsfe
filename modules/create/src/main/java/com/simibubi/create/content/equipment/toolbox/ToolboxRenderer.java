...
public class ToolboxRenderer extends SmartBlockEntityRenderer<ToolboxBlockEntity> {
	...
	protected void renderSafe(ToolboxBlockEntity blockEntity, float partialTicks, PoseStack ms,
		...
		SuperByteBuffer lid = CachedBufferer.partial(AllPartialModels.TOOLBOX_LIDS.get(blockEntity.getColor()),
				blockState);
		...
		lid.center().rotateY(-facing.toYRot()).uncenter().translate(0, 6 / 16f, 12 / 16f).rotateX(135 * lidAngle)
				.translate(0, -6 / 16f, -12 / 16f).light(light).renderInto(ms, builder);

		for (int offset : Iterate.zeroAndOne) {
			drawer.center().rotateY(-facing.toYRot()).uncenter()
					.translate(0, offset * 1 / 8f, -drawerOffset * .175f * (2 - offset)).light(light)
