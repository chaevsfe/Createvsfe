...
public abstract class Outline {
	...
	protected void putVertex(PoseStack.Pose pose, VertexConsumer builder, float x, float y, float z, float u, float v,
			Direction normal) {
		...
		builder.addVertex(pose.pose(), x, y, z)
				.setColor(rgb.getRedAsFloat(), rgb.getGreenAsFloat(), rgb.getBlueAsFloat(),
						rgb.getAlphaAsFloat() * params.alpha)
				.setUv(u, v).setOverlay(OverlayTexture.NO_OVERLAY).setLight(params.lightMap)
				.setNormal(pose, xOffset, yOffset, zOffset);
