...
public interface PotatoProjectileRenderMode {
	...
	public static class Tumble extends Billboard {
		...
		public void transform(PoseStack ms, PotatoProjectileEntity entity, float pt) {
			...
			TransformStack.of(ms).rotateZDegrees((entity.tickCount + pt) * 2 * entityRandom(entity, 16))
	...
	public static class TowardMotion implements PotatoProjectileRenderMode {
		...
		public void transform(PoseStack ms, PotatoProjectileEntity entity, float pt) {
			...
			TransformStack.of(ms).rotateYDegrees(AngleHelper.deg(Mth.atan2(diff.x, diff.z))).rotateXDegrees(
					270 + AngleHelper.deg(Mth.atan2(diff.y, -Mth.sqrt((float) (diff.x * diff.x + diff.z * diff.z)))));
			TransformStack.of(ms).rotateYDegrees((entity.tickCount + pt) * 20 * spin + entityRandom(entity, 360))
