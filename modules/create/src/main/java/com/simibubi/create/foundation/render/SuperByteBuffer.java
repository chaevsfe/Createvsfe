...
public class SuperByteBuffer implements TransformStack<SuperByteBuffer> {
	...
	public SuperByteBuffer(ByteBuffer vertexBuffer, MeshData.DrawState drawState) {
		int vertexCount = drawState.vertexCount();
		int stride = drawState.format().getVertexSize();
	...
	public void renderInto(PoseStack input, VertexConsumer builder) {
		...
		for (int i = 0; i < vertexCount; i++) {
			...
			if (hybridLight) {
				builder.setLight(maxLight(light, template.getLight(i)));
			} else {
	...
	public SuperByteBuffer shiftUV(SpriteShiftEntry entry) {
		this.spriteShiftFunc = (builder, u, v) -> {
			builder.setUv(entry.getTargetU(u), entry.getTargetV(v));
		};
		return this;
	}
