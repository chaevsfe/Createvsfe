...
public class ConfigScreenList extends ObjectSelectionList<ConfigScreenList.Entry> implements TickableGuiEventListener {
	...
	public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
		...
		UIRenderHelper.angledGradient(graphics, 180, getRowRight(), getX() + height / 2, height, 5, c, Color.TRANSPARENT_BLACK);

		super.render(graphics, mouseX, mouseY, partialTicks);
	}

	@Override
	protected void renderListItems(GuiGraphics graphics, int p_239229_, int p_239230_, float p_239231_) {
		Window window = minecraft.getWindow();
		double d0 = window.getGuiScale();
		RenderSystem.enableScissor((int) (this.getX() * d0), (int) (window.getHeight() - (this.getBottom() * d0)), (int) (this.width * d0), (int) (this.height * d0));
		super.renderListItems(graphics, p_239229_, p_239230_, p_239231_);
