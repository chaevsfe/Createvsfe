...
public class CarriageContraptionEntityRenderer extends ContraptionEntityRenderer<CarriageContraptionEntity> {
	...
	public void render(CarriageContraptionEntity entity, float yaw, float partialTicks, PoseStack ms,
		...
		carriage.bogeys.forEach(bogey -> {
			...
			if (!Backend.canUseInstancing(entity.level()) && !entity.getContraption().isHiddenInPortal(bogeyPos)) {
	...
	public static void translateBogey(PoseStack ms, CarriageBogey bogey, int bogeySpacing, float viewYRot,
		...
		TransformStack.of(ms).rotateY(viewYRot + 90).rotateX(-viewXRot).rotateY(180)
				.translate(0, 0, bogey.isLeading ? 0 : -bogeySpacing).rotateY(-180).rotateX(viewXRot)
				.rotateY(-viewYRot - 90).rotateY(bogey.yaw.getValue(partialTicks))
				.rotateX(bogey.pitch.getValue(partialTicks)).translate(0, .5f, 0).rotateZ(selfUpsideDown ? 180 : 0)
	...
	public static int getBogeyLightCoords(CarriageContraptionEntity entity, CarriageBogey bogey, float partialTicks) {
		...
		var lightPos = BlockPos.containing(Objects.requireNonNullElseGet(bogey.getAnchorPosition(),
