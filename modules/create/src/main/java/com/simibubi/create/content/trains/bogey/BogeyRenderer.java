...
/**
 * This is a port of the bogey api from Extended Bogeys, If you are looking to
 * implement your own bogeys you can find some helpful resources below:
 * <p>
 * - <a href=
 * "https://github.com/Rabbitminers/Extended-Bogeys/tree/1.18/multiloader/dev/common/src/main/java/com/rabbitminers/extendedbogeys/bogeys/styles">Extended
 * Bogeys (Examples)</a> - <a href=
 * "https://github.com/Rabbitminers/Extended-Bogeys/blob/1.18/multiloader/dev/API_DOCS.md">Extended
 * Bogeys (API documentation)</a> - <a href=
 * "https://github.com/Layers-of-Railways/Railway/tree/93e318d1e922b1e992b89b0aceef85a2d545f370/common/src/main/java/com/railwayteam/railways/content/custom_bogeys">Steam
 * n' Rails (Examples)</a>
 */
public abstract class BogeyRenderer {
	...
	/**
	 * A common interface for getting transform data for both in-world and
	 * in-contraption model data safely from a partial model
	 *
	 * @param model                  The key for the model data to instantiate or
	 *                               retrieve
	 ...
	 * @return A generic transform which can be used for both in-world and
	 *         in-contraption models
	 */
	public BogeyModelData[] getTransform(PartialModel model, PoseStack ms, boolean inInstancedContraption, int size) {
		return (inInstancedContraption) ? transformContraptionModelData(keyFromModel(model), ms)
				: createModelData(model, size);
	}

	/**
	 * A common interface for getting transform data for both in-world and
	 * in-contraption model data safely from a blockstate
	 *
	...
	/**
	 * Helper function to collect or create a single model from a partial model used
	 * for both in-world and in-contraption rendering
	 *
	 * @param model                  The key of the model to be collected or
	 *                               instantiated
	 * @param ms                     Posestack to bind the model to if it is within
	 *                               a contraption
	...
	/**
	 * A common interface for getting transform data for blockstates, for a single
	 * model
	 *
	 * @param state         The state of the model to be collected or instantiated
	 * @param ms            Posestack to bind the model to if it is within a
	 *                      contraption
	 ...
	 *
	 * @param bogeyData  Custom data stored on the bogey able to be used for
	 *                   rendering
	...
	/**
	 * Used for calling in-contraption rendering ensuring that falsey data is
	 * handled correctly
	 *
	 * @param bogeyData  Custom data stored on the bogey able to be used for
	 *                   rendering
	...
	/**
	 * Used to collect Contraption Model Data for in-contraption rendering, should
	 * not be utilised directly when rendering to prevent render type mismatch
	 *
	...
	/**
	 * Used for in world rendering, creates a set count of model data to be
	 * rendered, allowing for a generic response when rendering multiple models both
	 * in-world and in-contraption for example, with wheels
	 *
	...
	/**
	 * Utility function to clone in-world models to a set size to allow for common
	 * handling of rendering with multiple instances of the same model for example
	 * with wheels
	 *
	...
	private BogeyModelData[] expandArrayToLength(BogeyModelData[] data, int size) {
		return Arrays.stream(Collections.nCopies(size, data).toArray())
				.flatMap(inner -> Arrays.stream((BogeyModelData[]) inner)).toArray(BogeyModelData[]::new);
	}

	/**
	 * Provides render implementations a point in setup to instantiate all model
	 * data to be needed
	 *
	...
	/**
	 * Creates instances of models for in-world rendering to a set length from a
	 * provided partial model
	 *
	 * @param materialManager The material manager
	...
	/**
	 * Creates instances of models for in-contraption rendering to a set length from
	 * a provided blockstate
	 *
	 * @param materialManager The material manager
	...
	/**
	 * Creates a single instance of models for in-contraption rendering from a
	 * provided blockstate
	 *
	 * @param materialManager The material manager
	...
	/**
	 * Helper function to create a single model instance for in-contraption
	 * rendering
	 *
	 * @param materialManager The material manager
	...
	/**
	 * This method is deprecated, use BogeyModelData#render instead, left in to
	 * avoid existing usages from crashing
	 *
