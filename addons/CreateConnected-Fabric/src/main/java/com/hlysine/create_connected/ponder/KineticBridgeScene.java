...
public class KineticBridgeScene {
    public static void kineticBridge(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.overlay.showControls(new InputWindowElement(util.vector.blockSurface(bridgeSource, Direction.UP), Pointing.DOWN).rightClick(), 40);
        scene.idle(20);
        ...
        scene.idle(10);
        Emitter smoke = Emitter.withinBlockSpace(ParticleTypes.SMOKE, Vec3.ZERO);
        ...
        Emitter cloud = Emitter.withinBlockSpace(ParticleTypes.CLOUD, Vec3.ZERO);
