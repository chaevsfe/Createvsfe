...
public class BoilerData extends com.simibubi.create.content.fluids.tank.BoilerData {
    ...
    public boolean[] occludedDirections = new boolean[4];
    ...
    // client only sound control (SoundPool not available in Fabric; sounds are triggered inline)
    ...
    public void tick(FluidTankBlockEntity controller) {
        ...
        if (ticksUntilNextSample > 0)
            return;
        int capacity = (int) controller.getTankInventory().getCapacity();
        if (capacity == 0)
            return;
    ...
    public CompoundTag write() {
        ...
        nbt.putFloat("Supply", (float) waterSupply);
