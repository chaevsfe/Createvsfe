...
//	}
    ...
    private static class WrappedSoundEntry extends SoundEntry {
        ...
        @Override
        public void prepare() {
            // prepare is called before register; just set up the locations
        ...
        public void register() {
            for (CompiledSoundEvent compiledEvent : compiledEvents) {
                ResourceLocation location = compiledEvent.location();
                net.minecraft.core.Registry.register(net.minecraft.core.registries.BuiltInRegistries.SOUND_EVENT, location, SoundEvent.createVariableRangeEvent(location));
            }
        }

        @Override
        public Holder<SoundEvent> getMainEventHolder() {
            return Holder.direct(getMainEvent());
        }

        @Override
        public SoundEvent getMainEvent() {
            return net.minecraft.core.registries.BuiltInRegistries.SOUND_EVENT.get(compiledEvents.getFirst().location());
        }
        ...
        public void play(Level world, Player entity, double x, double y, double z, float volume, float pitch) {
            for (WrappedSoundEntry.CompiledSoundEvent event : compiledEvents) {
                SoundEvent se = net.minecraft.core.registries.BuiltInRegistries.SOUND_EVENT.get(event.location());
                if (se != null) world.playSound(entity, x, y, z, se, category, event.volume() * volume, event.pitch() * pitch);
            }
        }
        ...
        public void playAt(Level world, double x, double y, double z, float volume, float pitch, boolean fade) {
            for (WrappedSoundEntry.CompiledSoundEvent event : compiledEvents) {
                SoundEvent se = net.minecraft.core.registries.BuiltInRegistries.SOUND_EVENT.get(event.location());
                if (se != null) world.playLocalSound(x, y, z, se, category, event.volume() * volume, event.pitch() * pitch, fade);
            }
        }

        private record CompiledSoundEvent(ResourceLocation location, float volume, float pitch) {
        }
    ...
    private static class CustomSoundEntry extends SoundEntry {
        ...
        @Override
        public void prepare() {
            // Nothing needed before register()
        }

        @Override
        public void register() {
            net.minecraft.core.Registry.register(net.minecraft.core.registries.BuiltInRegistries.SOUND_EVENT, id, SoundEvent.createVariableRangeEvent(id));
        }
        ...
        @Override
        public SoundEvent getMainEvent() {
            return net.minecraft.core.registries.BuiltInRegistries.SOUND_EVENT.get(id);
        }
        ...
        @Override
        public void play(Level world, Player entity, double x, double y, double z, float volume, float pitch) {
            SoundEvent se = getMainEvent();
            if (se != null) world.playSound(entity, x, y, z, se, category, volume, pitch);
        }

        @Override
        public void playAt(Level world, double x, double y, double z, float volume, float pitch, boolean fade) {
            SoundEvent se = getMainEvent();
            if (se != null) world.playLocalSound(x, y, z, se, category, volume, pitch, fade);
        }
