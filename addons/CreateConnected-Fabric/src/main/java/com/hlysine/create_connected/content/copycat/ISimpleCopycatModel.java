    ...
    default void assemblePiece(List<BakedQuad> sourceQuads, List<BakedQuad> destQuads, int angle, boolean flipY, MutableVec3 offset, MutableAABB select, MutableCullFace cull) {
        ...
        for (BakedQuad quad : sourceQuads) {
            ...
            destQuads.add(BakedQuadHelper.cloneWithCustomGeometry(quad,
                    cropAndMoveInts(quad, select.toAABB(), offset.toVec3().subtract(select.minX / 16f, select.minY / 16f, select.minZ / 16f))));
        }
    }

    static int[] cropAndMoveInts(BakedQuad quad, AABB crop, Vec3 move) {
        ...
        float uScale = (float) Math.round((SpriteShiftEntry.getUnInterpolatedU(sprite, u3) - SpriteShiftEntry.getUnInterpolatedU(sprite, u0)) / xyz3.distanceTo(xyz0));
        float vScale = (float) Math.round((SpriteShiftEntry.getUnInterpolatedV(sprite, v1) - SpriteShiftEntry.getUnInterpolatedV(sprite, v0)) / xyz1.distanceTo(xyz0));
        if (uScale == 0) {
            ...
            uScale = (float) Math.round((SpriteShiftEntry.getUnInterpolatedU(sprite, u1) - SpriteShiftEntry.getUnInterpolatedU(sprite, u0)) / xyz1.distanceTo(xyz0));
            vScale = (float) Math.round((SpriteShiftEntry.getUnInterpolatedV(sprite, v3) - SpriteShiftEntry.getUnInterpolatedV(sprite, v0)) / xyz3.distanceTo(xyz0));
        }
        ...
        for (int vertex = 0; vertex < 4; vertex++) {
            ...
            if (diff.lengthSqr() > 0) {
                ...
                BakedQuadHelper.setU(vertexData, vertex, sprite.getU(SpriteShiftEntry.getUnInterpolatedU(sprite, u) + uDiff));
                BakedQuadHelper.setV(vertexData, vertex, sprite.getV(SpriteShiftEntry.getUnInterpolatedV(sprite, v) + vDiff));
            }
