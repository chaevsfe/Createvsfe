...
public abstract class PacketSet {
	private static final Map<ResourceLocation, PacketSet> S2C_REGISTRY = new HashMap<>();
	...
	protected PacketSet(String id, int version,
		...
		S2C_REGISTRY.put(s2cPacket, this);
	}

	/**
	 * Look up a PacketSet by its S2C channel ResourceLocation.
	 */
	public static PacketSet getByS2CChannel(ResourceLocation channel) {
		return S2C_REGISTRY.get(channel);
	}
	...
	public record CheckVersionPacket(int serverVersion) implements S2CPacket {
		...
		public void handle(Minecraft mc) {
			...
			mc.getConnection().onDisconnect(new net.minecraft.network.DisconnectionDetails(error));
		}
	}
