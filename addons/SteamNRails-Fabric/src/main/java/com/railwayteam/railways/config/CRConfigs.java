...
public class CRConfigs {
    ...
    public static CClient client() {
        if (client == null) registerCommon();
        return client;
    }

    public static CCommon common() {
        if (common == null) registerCommon();
        return common;
    }

    public static CServer server() {
        if (server == null) registerCommon();
        return server;
    }
    ...
    public static void registerCommon() {
        ...
        // Register with Forge Config API Port so configs actually load from TOML
        for (java.util.Map.Entry<ModConfig.Type, ConfigBase> pair : CONFIGS.entrySet()) {
            fuzs.forgeconfigapiport.fabric.api.neoforge.v4.NeoForgeConfigRegistry.INSTANCE.register(
                Railways.MOD_ID, pair.getKey(), pair.getValue().specification);
        }

        fuzs.forgeconfigapiport.fabric.api.neoforge.v4.NeoForgeModConfigEvents.loading(Railways.MOD_ID)
            .register(CRConfigs::onLoad);
        fuzs.forgeconfigapiport.fabric.api.neoforge.v4.NeoForgeModConfigEvents.reloading(Railways.MOD_ID)
            .register(CRConfigs::onReload);
    }
    ...
    public static boolean getDisableDatafixer() {
        if (common != null && common.specification != null && common.specification.isLoaded())
        ...
        return cachedDisableDatafixer != null ? cachedDisableDatafixer : false;
    }
