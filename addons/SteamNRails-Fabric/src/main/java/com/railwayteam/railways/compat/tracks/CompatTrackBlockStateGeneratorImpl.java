...
public class CompatTrackBlockStateGeneratorImpl extends CompatTrackBlockStateGenerator {
    ...
    public <T extends Block> ModelFile getModel(DataGenContext<Block, T> ctx, RegistrateBlockstateProvider prov, BlockState state) {
        ...
        for (String k : textureMap.keySet()) {
            builder = builder.texture(k, ResourceLocation.fromNamespaceAndPath(material.id.getNamespace(), prefix + textureMap.get(k) + material.resourceName()));
        }
        for (String k : new String[]{"segment_left", "segment_right", "tie"}) {
            prov.models()
                .withExistingParent(outputPrefix + k,
                    Create.asResource("block/track/" + k))
                .texture("0", ResourceLocation.fromNamespaceAndPath(material.id.getNamespace(), prefix + "standard_track_" + material.resourceName()))
                .texture("1", ResourceLocation.fromNamespaceAndPath(material.id.getNamespace(), prefix + "standard_track_mip_" + material.resourceName()))
                .texture("particle", material.particle);
        }
        return builder;
    }
