...
public class FuelTankItem extends BlockItem {
    ...
    protected boolean updateCustomBlockEntityTag(@NotNull BlockPos blockPos, Level level, Player player,
        ...
        if (minecraftserver == null)
            return false;
        CompoundTag nbt = itemStack.has(DataComponents.BLOCK_ENTITY_DATA) ? itemStack.get(DataComponents.BLOCK_ENTITY_DATA).getUnsafe() : null;
    ...
    private void tryMultiPlace(BlockPlaceContext ctx) {
        ...
        for (int xOffset = 0; xOffset < width; xOffset++) {
            for (int zOffset = 0; zOffset < width; zOffset++) {
                ...
                EntityUtils.getPersistentData(player)
