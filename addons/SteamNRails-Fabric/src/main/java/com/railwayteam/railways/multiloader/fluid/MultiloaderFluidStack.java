...
public abstract class MultiloaderFluidStack {
    ...
    public static MultiloaderFluidStack loadFluidStackFromNBT(CompoundTag tag) {
        // In 1.21.1, FluidStack uses DataComponents, not NBT. Parse basic fluid info.
        if (tag.contains("FluidName")) {
            String fluidName = tag.getString("FluidName");
            long amount = tag.contains("Amount") ? tag.getLong("Amount") : 0;
            net.minecraft.resources.ResourceLocation fluidId = net.minecraft.resources.ResourceLocation.parse(fluidName);
            Fluid fluid = net.minecraft.core.registries.BuiltInRegistries.FLUID.get(fluidId);
            return create(fluid, amount);
        }
        return EMPTY;
    }
